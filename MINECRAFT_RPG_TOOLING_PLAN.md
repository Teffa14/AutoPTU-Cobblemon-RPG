...
| WORLD-022 | LIVE | Camp interaction. PR #240 adds the physical campfire-over-barrel assessment surface. PR #241 / implementation head `fcc356e4f2260caf4e7d142a23a4c03d5bf02d23` adds the actual server-owned Ouros camp result keyed by dimension/block position and reuses the persisted result after reconnect/restart. Composite camp identity is migration debt under WORLD-028. |
...
| WORLD-026 | LIVE | Physical item storage terminal/menu — PR #262 / implementation head `ff27bfd65b8a0df7c093035847d2fb29b6bba094`. The authored terminal opens a 9x6 server-authored bag/storage menu, revalidates proximity and stale canonical quantities before each click, and delegates movement to SVC-037. Its current composite block signature is migration debt under WORLD-028. Stored items remain excluded from active bag sale/crafting/reservation reads. |
...
| WORLD-028 | NEXT/PARTIAL | Migrate remaining permanent composite facility signatures to explicit namespaced mod blocks/items and datapack-backed content. PR #280 migrates Cedar Mart to `autoptu_cobblemon_rpg_fabric_adapter:cedar_mart_counter` and removes the emerald-over-barrel authority signature. Next: migrate the item-storage terminal, then the crafting workstation and field camp. Keep native unique primitives such as Cobblemon PC/Healing Machine and Minecraft Lodestone where they already provide unambiguous identity. |
