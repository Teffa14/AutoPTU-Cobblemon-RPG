...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "20841745242df28ef2e6a5f0e6f593dbcdfb2547";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.ACTION_ECONOMY_AND_INITIATIVE, verified(
                "ActionBudget, typed turn phases, deterministic initiative, Trick Room/League ordering; Trainer AP is separately authoritative in TrainerRuntimeState. InitiativeProgressState stores server-owned initiative order/cursor and BattleRoundController owns lifecycle mutation. BattleRoundController.advanceInitiativeTurn advances the canonical cursor within the current round, skips missing/fainted/inactive slots, resets only the selected combatant's consumed actions, opens START, emits TurnStartedEvent, resolves START status/ability/perk hooks through the generic lifecycle dispatcher, then consumes any pending status skip before returning the autobattler decision window. Flinch coverage proves STANDARD/SHIFT are already consumed before AI can choose. Analytic resolves against authoritative defender ActionBudget history plus InitiativeProgressState with Python-parity strict cursor semantics",
                "Treat action availability, initiative, turn selection, START effect ordering and Trainer resource spending as core-owned state. The integration may render TurnStartedEvent and subsequent Java-emitted START effects/skips in sequence and freeze canonical Trainer AP at battle start, but must not choose the next actor, skip initiative slots, reset ActionBudget, replace initiative order, set the initiative cursor, execute START hooks, consume a status skip, spend/restore AP, or declare whether an Analytic target has already acted."));
        entries.put(Capability.FULL_TURN_ROUND_LIFECYCLE, partial(
                "BattleRoundController, ordered LifecycleHookRegistry, StatusPhaseEffectRegistry, AbilityPhaseEffectRegistry, PerkPhaseEffectRegistry, CombatantPhaseEffectDispatcher STATUS -> ABILITY -> PERK ordering, structured StatusStateStore, active actor/phase state, authoritative phase/turn-start/turn-end events, current-round advanceInitiativeTurn selection with Python parity, TURN_START START dispatcher execution before decision-window return, pending status-skip consumption after START effects, turn-end boundary, TemporaryEffectStore, move-frequency reset, selected round-start cleanup, canonical RoundInjuryHistoryState shared by BattleRuntimeState and BattleRoundController, server-owned BattleRuntimeState.currentRound synchronized only by lifecycle, server-owned InitiativeProgressState order/cursor synchronized only by lifecycle, damage/injury history rotation, delayed-hit scheduling/binding, Flinch behavior, Strange Tempo, Lancer END, authoritative TrainerRuntimeState/controller binding, mutable CombatStageState and parity-backed Defense Mastery/fixed Link Feature END behavior",
                "Consume verified lifecycle state/events only and preserve authoritative ordering, including turn_start -> START status/ability/perk semantic effects -> pending status_skip before any adapter decision UI opens. Actor selection, initiative cursor advancement, inactive/fainted skipping, action reset, START hook execution and skip consumption remain Java-owned. Automatic round rollover, trainer initiative entries, unported concrete START status/ability/perk implementations, AP spending, combat-stage mutation, broader concrete perk/status/ability libraries, delayed-hit execution, terrain/zone/room advancement, send-out effects and remaining Python lifecycle hooks remain core-owned. The adapter must never set or advance BattleRuntimeState.currentRound or mutate InitiativeProgressState."));
        ...
        entries.put(Capability.AI_LEGAL_ACTION_INFRASTRUCTURE, verified(
                "Runtime-authoritative autobattler action space, affiliation, geometry, action budget, move availability and frequency filtering; advanceInitiativeTurn resolves START effects and pending status skips before the AI receives its decision window",
                "AI may choose only from the legal choices produced by the core after authoritative START effects and skips have already resolved. The adapter must not expose a decision window from pre-START or pre-skip state."));
