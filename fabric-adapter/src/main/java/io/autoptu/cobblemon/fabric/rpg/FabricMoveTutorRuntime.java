 ...
 *
 * Minecraft Villager owns only the actor/model/interaction surface. AutoPTU resolves the
 * authenticated Trainer, party membership and current move loadout from canonical persistence.
 * No move may be taught, forgotten or relearned until upstream exposes authoritative PTU
 * learnability and mutation contracts.
 */
public final class FabricMoveTutorRuntime {
    ...
    private static final class TutorScreenHandler extends GenericContainerScreenHandler {
        ...
        public boolean canUse(PlayerEntity clickingPlayer) {
            if (clickingPlayer != player || player.isRemoved() || !(player.getWorld() instanceof ServerWorld serverWorld)) return false;
            Entity current = serverWorld.getEntity(npcEntityId);
            return current != null && !current.isRemoved() && isTutor(current)
        ...
        public void onSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity clickingPlayer) {
            ...
            Optional<CanonicalMoveTutorInspectionService.Inspection> current = inspectionService.inspect(playerId, selection.partySlot());
            if (current.isEmpty()
            ...
            player.sendMessage(Text.literal(
                    displayName(inspection.speciesId()) + " current moves: " + moves), false);
            player.sendMessage(Text.literal(
                    "Teaching, forgetting and relearning remain unavailable until AutoPTU supplies authoritative PTU learnability and mutation."), false);
        }
        ...
        private void refresh() {
            ...
            displayInventory.setStack(4, named(Items.ENCHANTED_BOOK.getDefaultStack(),
            ...
            for (int partySlot = 1; partySlot <= 6 && displaySlot <= 16; partySlot++) {
                ...
                String moves = inspection.moveLoadoutAvailable()
                        ? (inspection.currentMoveIds().isEmpty() ? "none" : String.join(", ", inspection.currentMoveIds()))
                        : "unavailable";
                displayInventory.setStack(displaySlot, named(Items.BOOK.getDefaultStack(),
            ...
            displayInventory.setStack(22, named(Items.BARRIER.getDefaultStack(),
