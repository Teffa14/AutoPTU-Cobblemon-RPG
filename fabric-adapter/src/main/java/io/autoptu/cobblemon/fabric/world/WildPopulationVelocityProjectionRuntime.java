 ...
 *
 * <p>Velocity, sprinting/sneaking/swimming, native pose, riding state, navigation, vanilla/Cobblemon targets, attacker memory,
 * vanilla fire, accumulated fall distance, vanilla hurt/death animation time, native air depletion, vanilla freezing, embedded
 * projectile counters, native glowing and native custom-name visibility are presentation/world state only. AutoPTU-Java
 * remains authoritative for tactical movement, targeting, forced movement, reactions, damage, statuses and battle outcomes.
 * Hidden or interaction-inactive actors must not keep following a stale path, drift away from their server-authored ecology
 * projection, remain attached to an unrelated Minecraft vehicle, retain native combat memory, keep vanilla damage/death
 * presentation alive, carry dormant physics into a later visible projection, surface with a depleted Minecraft air meter,
 * thaw into native freeze damage, reappear with stale arrow/stinger damage presentation, or retain stale native
 * locomotion/pose/glowing/nameplate state.</p>
 */
public final class WildPopulationVelocityProjectionRuntime implements ModInitializer {
    ...
    static boolean shouldDismountNativeVehicle(boolean interactionActive, boolean invisible, boolean hasVehicle) {
        return shouldSuspendPresentation(interactionActive, invisible) && hasVehicle;
    }
