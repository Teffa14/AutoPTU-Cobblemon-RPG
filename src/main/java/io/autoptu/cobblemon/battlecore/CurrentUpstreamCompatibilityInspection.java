...
public final class CurrentUpstreamCompatibilityInspection {
    public static final String AUTOPTU_JAVA_SHA = "46b8873df5839cca1b57106a16248c457d93f5fe";
    public static final String AUTOPTU_PYTHON_SHA = "91270f54b237e177fef46a875f5599e114db97e3";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.ABILITIES,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "Merged upstream PR #153 binds the declarative StatusAbilityPreventionResolver into live authoritative status application. RuntimeCombatantState owns abilitiesSuppressed; status requests cannot supply or override it. Inner Focus blocks Flinch, Immunity blocks poison, and Insomnia/Vital Spirit block sleep unless the target's abilities are suppressed. Merged PR #154 adds a separate Safeguard prevention contract but deliberately leaves runtime wiring for a later slice.",
                        "Own Tempo, Oblivious, Run Away, attacker-side ignore_defensive_abilities, Safeguard runtime wiring, and the sources that create/remove ability suppression remain outside this verified family. Python main still includes the Color Change same-type crash guard, but no Java parity for that behavior was verified in this run. The complete PTU ability library is not ported, and Minecraft must not decide status prevention or suppression, grant abilities or supply hook results."));

        result.put(UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "TrainerRuntimeState owns Feature identities, base AP, temporary AP grants, ActionBudget, initiative inputs, explicit initiative Speed, team identity, generic Feature resources and usage/cooldown bookkeeping. TrainerFeaturePrerequisiteResolution, TrainerFeatureContextResolution, TrainerFeatureFrequencyResolution, TrainerFeatureResourceResolution and TrainerFeatureUsageResolution are Python-parity backed. TrainerFeatureExecutionService.executeAuthoritative commits generic bookkeeping only after the effect reports applied. TrainerFeatureTargetResolution provides Python-parity generic Pokemon target scopes and TrainerFeatureTrainerTargetResolution provides parity-backed trainer-target scopes. TrainerFeatureEffectRegistry includes heal/heal_active, raise_cs, grant_temp_hp, grant_ap, apply_status and remove_status. The inspected Python oracle at 91270f54 still performs guards and target/effect execution before resource consumption and usage/cooldown bookkeeping.",
