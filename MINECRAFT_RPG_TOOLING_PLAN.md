...
| P0-012 | NEXT/PARTIAL | Post-battle commit | PR #305 / merge `c90a703479417fe84f8b0405532281310e5af1a2` persists one trusted engine-authored canonical Pokémon HP/status/injury final state against owner + frozen revision, survives repository restart, treats an exact immediate retry as idempotent, and rejects conflicting/stale writes. Continue with a durable multi-Pokémon/item transaction boundary; normal battle-loop invocation remains blocked until AutoPTU-Java exposes an explicit authoritative battle-finished/final-state handoff. |
