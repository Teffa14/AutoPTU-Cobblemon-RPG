...
public final class CurrentUpstreamCompatibilityInspection {
    ...
    public static final String AUTOPTU_PYTHON_SHA = "c3e67a718fca2d92ecc8316cfa98f757977f7986";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.COMPLETE_STATUS_LIFECYCLE,
                new Evidence(
                        ...
                        "Selected prevention is live, but complete status ticking, expiry, cures, remaining immunities, source-sensitive behavior and interactions are still partial. Draft upstream PR #154 freezes a Safeguard prevention contract but does not wire it into main runtime behavior. Minecraft may persist and transport authoritative status entries but must not interpret or execute missing status rules."));
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "Fabric 1.21.1 and Cobblemon 1.7.3 boot together in production-remapped dedicated-server CI. Live smokes verify UUID lookup, authoritative relocation, positive HP mirroring and public BATTLE_STARTED_PRE preemption. Player identity is authenticated through PlayerManager and durable player/profile/Pokemon state. ServerOwnedWildEncounterIdentityBinder accepts only battle/side/actor/Pokemon identity keys from the Cobblemon signal. ServerOwnedWildEncounterProvisioningService now creates deterministic canonical WILD participant/combatant IDs and encounter seed from server-owned encounter blueprints before that identity handoff, exposes the resulting CanonicalEncounterPokemonState through the canonical encounter repository contract, and keeps the external WILD actor ID as correlation data only. FileCanonicalPokemonRepository and FileCanonicalItemReservationRepository remain world-scoped durable authority stores; the two-process dedicated-server restart smoke verifies Trainer, encounter profile, Pokemon aggregate, item quantity and active item reservation across world reopen.",
                        "A successful authenticated graphical client encounter is still pending. The provisioning blueprint source still needs product wiring from trusted RPG/encounter generation data, and the deterministic provisioning seed is not yet composed into the later shared battle reservation seed. Encounter-profile creation, cross-aggregate transaction recovery, journaling, reservation expiry/reconciliation, RuntimeCombatantState materialization, zero-HP/faint presentation and complete battle playback remain pending. ServerPlayerEntity and Cobblemon state may provide identity/presentation inputs only and never PTU stats, inventory truth, modifiers, legality or outcomes."));
