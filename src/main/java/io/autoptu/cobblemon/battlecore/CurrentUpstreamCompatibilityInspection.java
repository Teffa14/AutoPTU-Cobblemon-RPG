...
public final class CurrentUpstreamCompatibilityInspection {
    ...
    public static final String AUTOPTU_PYTHON_SHA = "9df36aeae4bcbef49fd5edb658b51d68bd45fa71";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.ABILITIES,
                new Evidence(
                        ...
                        "Flower Veil is one bounded prevention family, not complete ability support. PR #158 for Big Pecks, Hyper Cutter, Clear Body, Full Metal Body and White Smoke is draft and is not part of the inspected main contract. Minecraft must not decide whether a stage drop is external, inspect type eligibility, calculate prevention ranges, choose the holder, apply or cancel a stage delta, or synthesize a combat_stage_block result. It may only render semantic events emitted by Java main."));

        result.put(UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "TrainerRuntimeState owns Feature identities, base AP, temporary AP grants, ActionBudget, initiative inputs, explicit initiative Speed, team identity, generic Feature resources and usage/cooldown bookkeeping. TrainerFeaturePrerequisiteResolution, TrainerFeatureContextResolution, TrainerFeatureFrequencyResolution, TrainerFeatureResourceResolution and TrainerFeatureUsageResolution are Python-parity backed. TrainerFeatureExecutionService.executeAuthoritative commits generic bookkeeping only after the effect reports applied. TrainerFeatureTargetResolution provides Python-parity generic Pokemon target scopes and TrainerFeatureTrainerTargetResolution provides parity-backed trainer-target scopes. TrainerFeatureEffectRegistry includes heal/heal_active, raise_cs, grant_temp_hp, grant_ap, apply_status and remove_status. The inspected Python main at 9df36aea has only a Career-domain change since cd2d31ab; trainer_features.py still performs guards and target/effect execution before resource consumption and usage/cooldown bookkeeping.",
