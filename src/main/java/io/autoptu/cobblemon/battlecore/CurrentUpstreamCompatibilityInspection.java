...
public final class CurrentUpstreamCompatibilityInspection {
    public static final String AUTOPTU_JAVA_SHA = "63526aa2dce83d0faa22b364705cd36f590d964b";
    public static final String AUTOPTU_PYTHON_SHA = "436b09ceb0811b74dc21924995aa82e56e581061";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "TrainerRuntimeState owns Feature identities, base AP, temporary AP grants, ActionBudget, initiative inputs, explicit initiative Speed, team identity, generic Feature resources and usage/cooldown bookkeeping. TrainerFeaturePrerequisiteResolution, TrainerFeatureContextResolution, TrainerFeatureFrequencyResolution, TrainerFeatureResourceResolution and TrainerFeatureUsageResolution are Python-parity backed. TrainerFeatureExecutionService.executeAuthoritative commits generic bookkeeping to TrainerRuntimeState only after the effect reports applied. TrainerFeatureTargetResolution provides Python-parity generic Pokemon target scopes. TrainerFeatureEffectRegistry on current main ports heal/heal_active, raise_cs and grant_temp_hp effect families against authoritative BattleRuntimeState, including combat-stage clamping and server-owned temporary HP grants.",
                        "Trainer Features remain partial. The current effect families do not cover the wider Python effect library, AP-specific costs, every trigger/lifecycle binding or complete Feature catalog. Temporary HP damage absorption still belongs to the incomplete stateful damage pipeline, and Trainer-target scopes/AP grants are not part of the inspected main head. Minecraft must not grant Features, decide gates, select or rewrite targets, execute Feature effects, mutate usage/cooldowns, spend resources/AP or report a Feature as applied."));
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK,
                new Evidence(
                        ...
                        "Live adapter support remains partial. The successful authenticated graphical client encounter is still pending. Durable encounter profiles persist canonical selection IDs and arena placement, not independent Pokemon/item aggregate truth; cross-aggregate transaction recovery and complete persistent asset stores remain pending. RuntimeCombatantState materialization, zero-HP/faint presentation, move animation, semantic cues, full entity lifecycle and complete battle playback remain pending. ServerPlayerEntity and Cobblemon entity state are identity/presentation inputs only and must never supply PTU stats, inventory truth, modifiers, legality or outcomes."));
