...
def cosmetic_bones() -> list[dict]:
    shoulder = v1.bone("ouros_v23_signature_shoulder_wrap", "shoulder_right", [-3.4, 29.6, 0.0], [
        shell((-8.15, 27.15, -1.85), (5.25, 1.05, 4.55), 80, pivot=(-4.7, 29.5, 0.2), rotation=(17, -18, 28), light=82, dark=89),
        shell((-7.05, 25.65, -0.10), (4.35, 0.82, 5.20), 81, pivot=(-4.45, 28.25, 0.85), rotation=(-8, -31, 19), light=81, dark=89),
        shell((-5.75, 24.15, 1.00), (3.45, 0.62, 4.80), 80, pivot=(-4.05, 26.75, 1.35), rotation=(-18, -22, 9), light=82, dark=90),
    ])
    back = v1.bone("ouros_v23_signature_back_flow", "torso3", [-2.35, 25.5, 1.2], [
        shell((-5.55, 22.25, 1.15), (3.85, 0.72, 5.55), 81, pivot=(-3.75, 25.8, 1.45), rotation=(13, -21, 14), light=81, dark=90),
        shell((-4.55, 18.45, 1.10), (3.05, 0.58, 5.15), 80, pivot=(-3.15, 22.5, 1.45), rotation=(7, -13, 5), light=82, dark=90),
        shell((-3.45, 15.05, 0.85), (2.25, 0.45, 4.35), 81, pivot=(-2.55, 19.1, 1.25), rotation=(-2, -7, -5), light=81, dark=89),
    ])
    hip = v1.bone("ouros_v23_signature_hip_fork", "torso", [-1.8, 16.0, 0.4], [
        shell((-4.0, 11.25, 0.25), (1.75, 0.38, 4.85), 80, pivot=(-2.55, 16.0, 0.65), rotation=(5, 11, -13), light=82, dark=90),
        shell((-1.15, 12.75, 0.75), (1.30, 0.32, 3.55), 81, pivot=(-0.85, 16.0, 0.85), rotation=(-5, -9, 11), light=81, dark=89),
    ])
    bridge = v1.bone("ouros_v23_signature_chest_bridge", "torso3", [0.0, 28.0, -2.35], [
        shell((-3.45, 26.55, -3.20), (5.25, 0.26, 1.05), 84, pivot=(-0.25, 28.3, -3.0), rotation=(4, 0, -24), light=86, dark=80),
    ])
...
def paint_pixel(r: int, g: int, b: int, a: int, x: int, y: int, *, shiny: bool):
    ...
    if blue:
        ...
        occ = max(0, (y - 28) // 9)
        if shiny:
            nr = int(r*.86) + 4; ng = int(g*.91) + 5 + facing//2; nb = int(b*.96) + 5 + facing
        else:
            nr = int(r*.72) + 5; ng = int(g*.82) + 7 + facing//2; nb = int(b*.94) + 8 + facing
...
def post_patch() -> None:
    ...
    converted = [x.replace("v22-derived-normal.json", "v23-derived-normal.json").replace("v22-derived-shiny.json", "v23-derived-shiny.json") for x in b["outputs"]]
    b["outputs"] = list(dict.fromkeys(converted))
    q = data["qualityIntent"]
    q["signaturePieces"] = ["Single asymmetric shoulder-to-back signature wrap", "Narrowing dorsal flow ending in a forked hip silhouette", "Small diagonal chest bridge plus restrained biological material repaint"]
    q["macroFormPlan"] = "V23 removes V22 crown, waistcoat, arm guards and isolated accessory language. Four contact-rooted systems form one continuous shoulder-back-hip envelope: three overlapping shoulder shells hand off into three narrowing dorsal shells, then two unequal hip tips create tail/leg negative space. One narrow chest bridge connects the asymmetric mass across the torso without covering the official spike."
    q["paintPlan"] = "Normal and shiny are independently derived from exact 1.7.3 baselines. Existing blue biology receives a restrained cobalt value pass with sparse cool facing highlights; existing dark biology gets indigo occlusion and edge breakup. Cream spikes, white landmarks, red eyes, dimensions, UV layout and alpha semantics stay unchanged."
    q["gameplayReadGoal"] = "At 160 px read one diagonal shoulder-back aura sheath with a clear taper and forked lower negative space, while the face, ears, chest spike, limbs and tail remain Lucario."
    q["iterationNote"] = "V22 engineering passed but direct review found crown/waistcoat/arm fragmentation, a backpack-like rear mass and insufficient silhouette integration. V23 deletes those systems rather than adding parts and concentrates silhouette budget into one contiguous wrap."
