...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "9e68bde8391b057c982900d038cc9ec3a3d348f9";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.FULL_STATEFUL_DAMAGE_PIPELINE, partial(
                "RuntimeMoveResolution, authoritative stats/types/statuses, ordered DamageModifierHookRegistry, pre-damage move hooks and ordered PostDamageHookRegistry/PostDamageHookResult; Burn, Pink Pearl, Mega Launcher, actual-HP-loss move-damage-history recording, and Aqua Boost/Ignition Boost/Thunder Boost post-result aura selection are parity-backed. BattleRuntime now applies resolved post-damage flat bonuses before authoritative HP mutation, MoveResolvedEvent emission and move-damage-history recording",
                "Consume only core-resolved damage, target HP and ordered semantic events. Minecraft may mirror MoveResolvedEvent damage/targetHp after authoritative mutation, but must never select aura sources, calculate or apply post-damage bonuses, inject unported move/ability/item/terrain modifiers, infer missing damage-history semantics, or perform an independent HP mutation."));
        ...
        entries.put(Capability.ABILITIES, partial(
                "Canonical ability identities, generic HookSource support, AbilityPhaseEffectRegistry, CombatStageHookRegistry, PostDamageHookRegistry and parity-backed Mega Launcher, Strange Tempo, Inner Focus, Lancer, Simple, Defiant, Competitive, Plus [SwSh], Minus [SwSh], Aqua Boost, Ignition Boost and Thunder Boost behavior; spatial abilities use authoritative positions, team identity, active/HP state, deterministic holder order and recursive hook suppression where applicable",
                "Render only authoritative ability events/results already emitted by the core. Combat-stage reactions, final stage values, radius/adjacency holder selection, team filtering, damage-bonus source selection, recursive mutation and hook suppression must be projected from core results rather than recomputed from Minecraft entities or requested deltas. Aura-adjusted damage/HP may be mirrored only from the final MoveResolvedEvent; do not apply aura bonuses or implement the remaining ability library in Minecraft."));
