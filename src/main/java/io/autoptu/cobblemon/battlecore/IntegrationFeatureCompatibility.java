...
public final class IntegrationFeatureCompatibility {
    ...
    private static Map<Feature, Requirement> buildRequirements() {
        ...
        requirements.put(Feature.LIVE_MINECRAFT_BATTLE_ADAPTER, requirement("Runtime-tested support currently covers production Fabric/Cobblemon startup, opaque PokemonEntity UUID lookup, an entity-bound authoritative relocation whose final server position is verified, positive HP projection as write-through presentation, and highest-priority public battle-trigger interception that claims only an opaque battle ID and cancels a real Cobblemon start before BattleRegistry or the post-start event. AutoPTU reservation creation from intercepted participants, zero-HP/faint presentation, move animation, semantic cues and full battle playback remain outside this bounded slice.", UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK));
