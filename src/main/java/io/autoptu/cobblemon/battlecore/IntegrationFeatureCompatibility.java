...
public final class IntegrationFeatureCompatibility {
    public enum Feature {
        ...
        PLAYER_VS_WILD_AUTHORITY_COMPOSITION,
        AUTHENTICATED_PLAYER_CONTEXT_RESOLUTION,
        SERVER_OWNED_WILD_ENCOUNTER_PROVISIONING,
        ...
        CANONICAL_INJURY_SNAPSHOT,
        CANONICAL_INJURY_BOOTSTRAP,
    ...
    private static Map<Feature, Requirement> buildRequirements() {
        ...
        requirements.put(Feature.PLAYER_VS_WILD_AUTHORITY_COMPOSITION, requirement("Compose canonical player Trainer/Pokemon/item/arena authority with canonical PLAYER/WILD battle affiliation under one server-issued reservation ID and RNG seed. Require exact player roster equality and compensate the player reservation if the encounter lock fails. This freezes identities and reserved resources only; it does not execute item effects, Trainer Features, movement, damage, status, ability or terrain mechanics.", UpstreamCompatibilityMatrix.Capability.ITEMS, UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS, UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK));
        requirements.put(Feature.AUTHENTICATED_PLAYER_CONTEXT_RESOLUTION, requirement("Treat the Cobblemon PLAYER actor UUID as an external identity only. Require that exact canonical UUID to map to a currently connected ServerPlayerEntity through MinecraftServer PlayerManager before a server-owned canonical encounter context may be read. Minecraft presence proves authentication only; ServerPlayerEntity stats, inventory, permissions, position and other mutable fields never become PTU authority.", UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK));
        requirements.put(Feature.SERVER_OWNED_WILD_ENCOUNTER_PROVISIONING, requirement("Create canonical WILD participant/combatant identities and a deterministic encounter seed from trusted server-owned RPG encounter blueprints before Cobblemon battle-start interception. The external WILD actor UUID is correlation only. Canonical Pokemon values may be frozen and exposed to the reservation repository, but this boundary executes no movement, targeting, damage, status, ability, item or AI rules and never reads trusted values from PokemonEntity.", UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK));
        ...
        requirements.put(Feature.CANONICAL_INJURY_SNAPSHOT, requirement("Freeze only the persistent server-owned PTU injury count into the immutable Pokemon battle snapshot. Injury generation, healing/rest, Aura Storm scaling, Aura Break blocking/inversion and all injury-sensitive effects remain core/domain-owned; legacy records carry no trusted injury value.", UpstreamCompatibilityMatrix.Capability.FULL_STATEFUL_DAMAGE_PIPELINE, UpstreamCompatibilityMatrix.Capability.ABILITIES));
        requirements.put(Feature.CANONICAL_INJURY_BOOTSTRAP, requirement("Bind frozen injury counts to the exact authoritative battle reservation and roster, then expose current counts only for AutoPTU-Java BattleRuntimeState.injuryHistory()/RoundInjuryHistoryState initialization. Previous/last-round history, lifecycle rotation, later injury mutation, Aura Storm arithmetic and Aura Break decisions remain core-owned; Minecraft/client payloads may not supply any of them.", UpstreamCompatibilityMatrix.Capability.FULL_STATEFUL_DAMAGE_PIPELINE, UpstreamCompatibilityMatrix.Capability.ABILITIES, UpstreamCompatibilityMatrix.Capability.FULL_TURN_ROUND_LIFECYCLE));
        ...
        requirements.put(Feature.CANONICAL_TRAINER_FEATURE_BOOTSTRAP, requirement("Bind the frozen Trainer Feature identity set to the same battle reservation and trainer identity expected by the upstream PerkPhaseEffectRegistry projection. Transport identity only; feature execution remains AutoPTU-Java-owned.", UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS, UpstreamCompatibilityMatrix.Capability.FULL_TURN_ROUND_LIFECYCLE));
        requirements.put(Feature.CANONICAL_TRAINER_RUNTIME_BOOTSTRAP, requirement("Freeze server-owned battle-start Trainer AP, canonical Trainer initiative modifier and Trainer Feature ownership, then derive controlled combatant IDs exclusively from the canonical reserved Pokemon roster. This prepares AutoPTU-Java TrainerRuntimeState plus BattleRuntimeState putTrainer/bindController without allowing Minecraft or clients to set AP or initiative modifiers, grant Features, select controllers, calculate initiative totals, spend/restore AP or execute perk effects.", UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS, UpstreamCompatibilityMatrix.Capability.FULL_TURN_ROUND_LIFECYCLE, UpstreamCompatibilityMatrix.Capability.ACTION_ECONOMY_AND_INITIATIVE));
        ...
        requirements.put(Feature.RUNTIME_BATTLE_PREPARATION_ENVELOPE, requirement("Bind materialization inputs, authoritative move metadata, frozen held-item identities and ordered structured status metadata to one reservation and roster while preserving the explicit AutoPTU-Java-owned blockers. Status metadata may feed the upstream BattleRuntimeState/StatusStateStore contract, but status interpretation and lifecycle remain core-owned. This envelope still must not construct RuntimeCombatantState or invent MovementProfile, ActionBudget, dynamic accuracy/evasion flags, damage modifiers, move effects, ability effects, item effects, status expiry, cures or ticks.", UpstreamCompatibilityMatrix.Capability.CORE_TARGETING, UpstreamCompatibilityMatrix.Capability.CORE_MOVEMENT_LEGALITY, UpstreamCompatibilityMatrix.Capability.CORE_CALCULATIONS_AND_COMBAT_STATS, UpstreamCompatibilityMatrix.Capability.MOVE_SPECIFIC_BEHAVIOR, UpstreamCompatibilityMatrix.Capability.ABILITIES, UpstreamCompatibilityMatrix.Capability.ITEMS, UpstreamCompatibilityMatrix.Capability.COMPLETE_STATUS_LIFECYCLE, UpstreamCompatibilityMatrix.Capability.FULL_TURN_ROUND_LIFECYCLE));
        requirements.put(Feature.RUNTIME_RULE_STATE_SEED, requirement("Combine one reservation-scoped prepared battle with canonical current injury counts and already-resolved server-owned environment semantics: weather, PTU terrain identity, Tailwind teams, grounded state and mounted rider relationships. AutoPTU-Java owns battle round, injury-history rotation, initiative order/cursor, Trainer action buckets, environment lifecycle/mutation, Aura Break blocker selection, Aura Storm resolution and every subsequent hook decision; none may be supplied or derived by Minecraft.", UpstreamCompatibilityMatrix.Capability.FULL_STATEFUL_DAMAGE_PIPELINE, UpstreamCompatibilityMatrix.Capability.ABILITIES, UpstreamCompatibilityMatrix.Capability.FULL_TURN_ROUND_LIFECYCLE, UpstreamCompatibilityMatrix.Capability.TERRAIN_WEATHER_HAZARDS_ZONES_REACTIONS, UpstreamCompatibilityMatrix.Capability.ACTION_ECONOMY_AND_INITIATIVE));
        ...
        requirements.put(Feature.LIVE_MINECRAFT_BATTLE_ADAPTER, requirement("Runtime-tested support covers production Fabric/Cobblemon startup, opaque PokemonEntity UUID lookup, an entity-bound authoritative relocation whose final server position is verified, positive HP projection as write-through presentation, highest-priority public battle-trigger interception, and a real MinecraftServer PlayerManager authentication boundary that rejects offline or malformed PLAYER identities before canonical PTU context lookup. The live interception maps opaque actor/Pokemon UUIDs to canonical participant/combatant IDs and atomically reserves opposing rosters before cancellation. Player-versus-wild Trainer/item/arena composition is contract-tested under one server-issued identity/seed; server-owned WILD encounter provisioning now supplies canonical WILD identities/state without reading PokemonEntity values; successful authenticated graphical-player runtime remains pending. RuntimeCombatantState materialization, zero-HP/faint presentation, move animation, semantic cues and full battle playback remain outside this bounded slice.", UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK));
