...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "201e52e68184b52b14a5040f8a440058e6d8daa9";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.ACTION_ECONOMY_AND_INITIATIVE, verified(
                "ActionBudget, typed turn phases, deterministic initiative, Trick Room/League ordering; Trainer AP is separately authoritative in TrainerRuntimeState. InitiativeProgressState stores server-owned initiative order/cursor and BattleRoundController owns lifecycle mutation. BattleRoundController.advanceInitiativeTurn now advances the canonical cursor within the current round, skips missing/fainted/inactive slots, resets only the selected combatant's consumed actions, opens START, and emits TurnStartedEvent with authoritative initiativeIndex. Analytic resolves against authoritative defender ActionBudget history plus InitiativeProgressState with Python-parity strict cursor semantics",
                "Treat action availability, initiative, turn selection and Trainer resource spending as core-owned state. The integration may render TurnStartedEvent and freeze canonical Trainer AP at battle start, but must not choose the next actor, skip initiative slots, reset ActionBudget, replace initiative order, set the initiative cursor, spend/restore AP, or declare whether an Analytic target has already acted."));
        entries.put(Capability.FULL_TURN_ROUND_LIFECYCLE, partial(
                "BattleRoundController, ordered LifecycleHookRegistry, StatusPhaseEffectRegistry, AbilityPhaseEffectRegistry, PerkPhaseEffectRegistry, CombatantPhaseEffectDispatcher STATUS -> ABILITY -> PERK ordering, structured StatusStateStore, active actor/phase state, authoritative phase/turn-start/turn-end events, current-round advanceInitiativeTurn selection with Python parity, pending status-skip consumption, turn-end boundary, TemporaryEffectStore, move-frequency reset, selected round-start cleanup, canonical RoundInjuryHistoryState shared by BattleRuntimeState and BattleRoundController, server-owned BattleRuntimeState.currentRound synchronized only by lifecycle, server-owned InitiativeProgressState order/cursor synchronized only by lifecycle, damage/injury history rotation, delayed-hit scheduling/binding, Flinch behavior, Strange Tempo, Lancer END, authoritative TrainerRuntimeState/controller binding, mutable CombatStageState and parity-backed Defense Mastery/fixed Link Feature END behavior",
                "Consume verified lifecycle state/events only and preserve authoritative ordering. TurnStartedEvent may be rendered, but actor selection, initiative cursor advancement, inactive/fainted skipping and action reset remain Java-owned. Automatic round rollover, trainer initiative entries, broader START-phase processing, AP spending, combat-stage mutation, perk execution, injury-history rotation, broader concrete perk/status/ability libraries, delayed-hit execution, terrain/zone/room advancement, send-out effects and remaining Python lifecycle hooks remain core-owned. The adapter must never set or advance BattleRuntimeState.currentRound or mutate InitiativeProgressState."));
