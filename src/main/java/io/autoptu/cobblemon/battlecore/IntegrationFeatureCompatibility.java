...
public final class IntegrationFeatureCompatibility {
    ...
    private static Map<Feature, Requirement> buildRequirements() {
        ...
        requirements.put(Feature.LIVE_MINECRAFT_BATTLE_ADAPTER, requirement("Runtime-tested support covers production Fabric/Cobblemon startup, opaque PokemonEntity UUID lookup, an entity-bound authoritative relocation whose final server position is verified, positive HP projection as write-through presentation, and highest-priority public battle-trigger interception. The live interception maps opaque actor/Pokemon UUIDs to canonical participant/combatant IDs and atomically reserves opposing rosters before cancellation. Player-versus-wild Trainer/item/arena composition is contract-tested under one server-issued identity/seed but still lacks an authenticated graphical player runtime. RuntimeCombatantState materialization, zero-HP/faint presentation, move animation, semantic cues and full battle playback remain outside this bounded slice.", UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK));
