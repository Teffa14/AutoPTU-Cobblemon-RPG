...
| WORLD-007 | LIVE | Shop counter and buy/sell menu — PR #256 / implementation head `b483936f4bae43dd8c1eca77a1ecbd2d8ee8a6f6` established the canonical UI and transactions. PR #280 replaces the incidental emerald-over-barrel identity with the explicit namespaced `autoptu_cobblemon_rpg_fabric_adapter:cedar_mart_counter`; durable wallet, stock, bag and transaction services remain server-authoritative. |
...
| WORLD-013 | NEXT/PARTIAL | Region/grass ecology provisioning for visible roaming wild Pokémon — PR #457 / merge `0aa144c0415509e3d92b3191379b294539223e2c` supplies the first trusted complete Marea Fletchling blueprint and presentation projection. PR #458 / production head `d5ecc49760e954baf1d6da23f92345c379de704f` provisions/reuses the first actor on normal server startup with the blueprint published before reveal. PR #460 / production head `9361935988e9ab30eebbd1c805d0f100392c9e0a` adds normal Marea presence reconciliation, loaded/searchable habitat containment, stale UUID eviction on replacement, and a real Fabric + Cobblemon loss/replacement smoke. PR #461 / production head `06ecb30301aafa75e7584ff15c02ba355e37cc7e` expands the normal Marea population to two independently-authored visible Fletchling members with distinct canonical encounter identities, contexts and positions while reusing only the already-approved complete PTU profile; the real Fabric + Cobblemon smoke proves discarding one member creates a clean replacement binding while the second preserves its exact UUID/binding. PR #462 / production head `3fe753d34f9fbd30eebbd1c805d0f100392c9e0a` adds a second independently-authored Marea population at `ouros.marea.sendero_crossing`, so normal startup now projects three separately-bound actors across two canonical populations while reusing only the already-approved complete Fletchling PTU profile. Missing additional complete WILD blueprints block activation of additional species/populations only; they do not block generic ecology infrastructure. |
...
| RPG-003 | LIVE/PARTIAL | Quest objective event processing — PR #380 / merge `69d752d06941cf49250e34f6385df90110515789` persists owner-scoped objective progress from server-observed Cedar Meadow events. Additional authored objective event types remain follow-up work. |
...
| SVC-003 | LIVE/PARTIAL | `canUse(player, item, target, context)` — PR #607 established the reusable server-owned preflight over canonical Trainer identity, exact item ownership, available quantity, transaction locks and server-observed target/context. PRs #608-#612 moved that boundary into the normal canonical bag surface with clickable selection, physical Bundle quick access, a read-only paged Minecraft inventory UI and server-authored item presentation. PR #613 / production head `afc2170d245e84dba84c12dbadcb9d6c87440bde` adds fresh server-authored ready/blocked item-use preflight state to every displayed bag stack while every click still re-resolves the canonical stack and runs `canUse` again. PTU item effects, battle targeting/action economy, healing, capture, RNG and outcomes remain upstream-owned. |
...
| SVC-011 | LIVE/PARTIAL | `canAdvanceQuest(player, event)` — PR #380 / merge `69d752d06941cf49250e34f6385df90110515789` advances only accepted server-authored objectives from known server-observed event keys. Additional authored event/prerequisite types remain follow-up work. |
...
| SVC-013 | LIVE/PARTIAL | Item reservation/commit/rollback infrastructure exists. |
| SVC-014 | LIVE/PARTIAL | Encounter reservation infrastructure exists. |
| SVC-015 | LIVE/PARTIAL | Battle outcome commit infrastructure exists. |
| SVC-016 | LIVE | `credit/debit(transactionId, player, amount, source)` |
| SVC-017 | LIVE/PARTIAL | Quest reward commit/idempotency |
| SVC-018 | LIVE/PARTIAL | Persistent world-object mutation/idempotency |
| SVC-019 | BLOCKED/PARTIAL | Reconnect/restart active-session recovery. |
| SVC-020 | BLOCKED/PARTIAL | Server-only PTU world-action usage reservation. |
| SVC-021 | LIVE/PARTIAL | `assessWorldTask(player, task, context)` |
| SVC-022 | LIVE | `craftWorldTask(attemptId, player, recipe, quantity)` |
| SVC-023 | LIVE | `assessCraftMaterials(player, recipe, quantity)` |
| SVC-024 | LIVE | `depositCraftIngredient(player, serverObservedStack)` |
| SVC-025 | LIVE | `establishFieldCamp(attemptId, campId, player, task)` |
| SVC-026 | LIVE | `inspectBag(player)` / `inspectItem(player, itemKey)` |
| SVC-027 | LIVE | `inspectWallet(player)` |
| SVC-028 | LIVE | `inspectShop(player, shopId)` |
| SVC-029 | LIVE | `purchase(...)` |
| SVC-030 | LIVE | `inspectPokemonStorage(player)` |
| SVC-031 | LIVE | `transferPokemonStorage(...)` |
| SVC-032 | LIVE | `sell(...)` |
| SVC-033 | LIVE/PARTIAL | `dialogue(...)` |
| SVC-034 | LIVE | `acceptQuest(...)` |
| SVC-035 | LIVE | `inspectQuestJournal(...)` |
| SVC-036 | LIVE | `trackQuest(...)` |
| SVC-037 | LIVE | `inspectItemStorage(...)` |
| SVC-038 | LIVE | `requestTrainerChallenge(...)` |
| SVC-039 | LIVE/PARTIAL | `inspectTrainerProgression(...)` |
| SVC-040 | LIVE/PARTIAL | `inspectPokemonProgression(...)` |
| SVC-041 | LIVE/PARTIAL | NPC relationship inspection |
| SVC-042 | LIVE/PARTIAL | faction reputation inspection |
| SVC-043 | LIVE/PARTIAL | rival inspection |
| SVC-044 | LIVE/PARTIAL | Trainer records inspection |
| SVC-045 | LIVE/PARTIAL | world story inspection/mutation |
| SVC-046 | LIVE/PARTIAL | mail inspection/read/claim |
| SVC-047 | LIVE/PARTIAL | league registration |
| SVC-048 | LIVE/PARTIAL | move tutor inspection |
| SVC-049 | LIVE/PARTIAL | progression gate check |
| SVC-050 | LIVE/PARTIAL | Trainer profile inspection/mutation |
| SVC-051 | LIVE/PARTIAL | shop stock reconciliation |
...
| LIVE | Explicit long-term active-party + box/storage aggregate. |
| LIVE | Starter/onboarding claim state. |
| LIVE/PARTIAL | Trainer presentation/profile data. |
| LIVE/PARTIAL | Trainer XP/level/progression. |
| LIVE/PARTIAL | Pokémon XP/progression/evolution choices. |
| BLOCKED/PARTIAL | Canonical inventory quantities and wallet. |
| LIVE | Durable canonical item storage and transfer journal. |
| LIVE | Durable Minecraft-to-canonical crafting ingredient handoff journal. |
| LIVE | Durable canonical wallet transaction receipts. |
| LIVE | Durable canonical shop stock. |
| LIVE | Durable canonical shop purchase journal. |
| LIVE | Durable canonical shop sale journal. |
| LIVE | Durable canonical party/box transfer journal. |
| LIVE/PARTIAL | Quest journal/objectives/reward claims. |
| LIVE/PARTIAL | NPC relationships/reputation. |
| LIVE/PARTIAL | Faction reputation. |
| LIVE/PARTIAL | Rival narrative state. |
| LIVE/PARTIAL | Trainer records. |
| LIVE/PARTIAL | Discovery/world-event/story flags. |
| LIVE/PARTIAL | Durable RPG mail read/claim state. |
| LIVE/PARTIAL | Nursery custody. |
| LIVE/PARTIAL | Gym/League registration. |
| LIVE/PARTIAL | Fast-travel unlocks/checkpoints. |
| LIVE/PARTIAL | Active encounter session journal. |
| BLOCKED | Active battle checkpoint/recovery journal. |
| LIVE/PARTIAL | Post-battle result commit ledger/idempotency keys. |
| BLOCKED/PARTIAL | Trainer PTU Daily action usage and monotonic RPG-day state. |
| LIVE | Durable world-task/craft attempt ledger. |
| LIVE | Durable physical field-camp result ledger. |
