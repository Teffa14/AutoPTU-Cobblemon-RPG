...
public final class IntegrationFeatureCompatibility {
    ...
    private static Map<Feature, Requirement> buildRequirements() {
        ...
        requirements.put(Feature.RUNTIME_BATTLE_PREPARATION_ENVELOPE, requirement("Bind materialization inputs, authoritative move metadata and frozen held-item identities to one reservation and roster while preserving the explicit AutoPTU-Java-owned blockers. This envelope prepares data only; it must not construct RuntimeCombatantState or invent MovementProfile, ActionBudget, dynamic accuracy/evasion flags, damage modifiers, move effects, ability effects or item effects.", UpstreamCompatibilityMatrix.Capability.CORE_TARGETING, UpstreamCompatibilityMatrix.Capability.CORE_MOVEMENT_LEGALITY, UpstreamCompatibilityMatrix.Capability.CORE_CALCULATIONS_AND_COMBAT_STATS, UpstreamCompatibilityMatrix.Capability.MOVE_SPECIFIC_BEHAVIOR, UpstreamCompatibilityMatrix.Capability.ABILITIES, UpstreamCompatibilityMatrix.Capability.ITEMS));
        ...
        requirements.put(Feature.TRAINER_FEATURE_PLAYBACK, requirement("Only selected verified Trainer Feature behavior/events may be projected.", UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS));
