...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "dc8cc6677dcfcf830fb176458b05ad08dba9b526";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.FULL_STATEFUL_DAMAGE_PIPELINE, partial(
                "RuntimeMoveResolution, authoritative stats/types/statuses, ordered DamageModifierHookRegistry, pre-damage move hooks and ordered PostDamageHookRegistry/PostDamageHookResult with signed post-damage adjustments; Burn, Pink Pearl, Mega Launcher, actual-HP-loss move-damage-history recording, Aqua Boost/Ignition Boost/Thunder Boost, Power Spot and Type Aura post-result aura selection are parity-backed. Normal Aura Storm is live-wired through canonical MoveSpec Aura keywords, BattleRuntimeState.injuryHistory current injuries and the authoritative global AuraBreakBlockerQuery. Aura Storm [Errata] is now live-wired through canonical currentRound, current injuries and RuntimeCombatantState TemporaryEffectStore aura_break_errata entries via AuraBreakErrataAdjustment; active inversion emits Aura Break [Errata] then Aura Storm [Errata] semantic events, expired entries are cleared by the core, and the signed adjustment is applied before authoritative HP mutation, MoveResolvedEvent emission and move-damage-history recording",
                "Consume only core-resolved damage, target HP and ordered semantic events. Current injury count may be initialized only from the frozen canonical battle reservation into BattleRuntimeState.injuryHistory(); previous/last-round injury history and all later injury mutation remain core-owned. Minecraft must never set currentRound, determine Aura Break blockers or inversion, inspect/create/expire/clear aura_break_errata temporary effects, select aura sources, calculate signed post-damage adjustments, inject unported move/ability/item/terrain modifiers, infer missing damage-history semantics, or perform an independent HP mutation."));
