...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "2543881dd5863d7a4b01336dadc24a3be40fd6aa";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.FULL_STATEFUL_DAMAGE_PIPELINE, partial(
                "RuntimeMoveResolution, authoritative stats/types/statuses, ordered DamageModifierHookRegistry, pre-damage move hooks and ordered PostDamageHookRegistry/PostDamageHookResult; Burn, Pink Pearl, Mega Launcher, actual-HP-loss move-damage-history recording, and Aqua Boost/Ignition Boost/Thunder Boost post-result aura bonus selection are parity-backed, but post-damage hook bonuses are not yet wired into BattleRuntime final HP mutation",
                "Consume only resolved damage and semantic events. Never inject unported move/ability/item/terrain modifiers, infer non-move damage-history semantics, apply post-damage aura bonuses, or mutate final HP in the adapter while the authoritative post-damage result is not yet connected to runtime HP mutation."));
        ...
        entries.put(Capability.ABILITIES, partial(
                ...
                "Render only authoritative ability events/results already emitted by the core. Combat-stage reactions, final stage values, radius/adjacency holder selection, team filtering, damage-bonus source selection, recursive mutation and hook suppression must be projected from core results rather than recomputed from Minecraft entities or requested deltas. Do not apply aura damage bonuses or implement the remaining ability library in Minecraft."));
