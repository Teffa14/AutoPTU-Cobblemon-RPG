...
public final class UpstreamCompatibilityMatrix {
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.TRAINER_FEATURES_AND_PERKS, partial(
                "Authoritative TrainerRuntimeState stores server-owned Trainer Feature identities, AP and initiativeModifier; BattleRuntimeState binds combatants to trainer controllers; PerkPhaseLifecycleHook derives owned Features from runtime state; PerkPhaseEffectRegistry participates in TURN_START START dispatch and PHASE_CHANGE; CombatStageState/CombatStageHookRegistry are authoritative reaction seams; selected status-skip exceptions, Defense Mastery and fixed Link Feature END behavior are parity-backed. TrainerFeaturePrerequisiteResolution now ports Python-parity feature/edge/known-feature discovery plus class, subclass, level and required-feature prerequisite gates",
                "Freeze and transport Trainer Feature ownership, battle-start AP, canonical initiative modifier and combatant-controller bindings only from server state. Minecraft/client payloads may not grant Features, decide prerequisites, set AP or initiative modifiers, spend/restore AP, choose controllers, execute perk hooks, mutate combat stages or run reactions. Context conditions, frequency/cooldowns, resource consumption, usage mutation, targeting and the remaining Trainer Feature/perk implementations stay AutoPTU-Java-owned."));
        ...
        entries.put(Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK, partial(
                "Production-remapped Fabric 1.21.1 + Cobblemon 1.7.3 dedicated-server startup is verified. Server-side UUID lookup resolves live PokemonEntity handles, and the runtime relocation smoke binds a real PokemonEntity through PresentationEntityHandleRegistry/RegistryBackedPresentationEntityGateway, applies an authoritative WorldBlockCoordinate relocation and verifies the resulting server position",
                "Keep support partial. HP projection, move animation, semantic cues, battle-trigger interception, full entity lifecycle and complete battle playback remain unverified. Cobblemon entity state is presentation-only and must never supply PTU stats, legality or outcomes."));
