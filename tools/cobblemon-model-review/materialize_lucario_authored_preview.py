...
def solid_uv(u:int,v:int)->dict:
    face={"uv":[u,v],"uv_size":[1,1]}; return {k:dict(face) for k in ("north","east","south","west","up","down")}
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--patch",type=Path,required=True); ap.add_argument("--official-model",type=Path,required=True); ap.add_argument("--official-texture",type=Path,required=True); ap.add_argument("--output-dir",type=Path,required=True)
    ...
    official_names={b.get("name") for b in bones}; candidate=copy.deepcopy(src); cgeo=candidate["minecraft:geometry"][0]; cgeo["description"]["identifier"]=f"geometry.ouros_{stem}"; added=[]
    for bi,b in enumerate(patch["bones"]):
        ...
        for ci,c in enumerate(b["cubes"]):
            slot=(bi+ci)%len(PALETTE); u=80+slot; v=63; nc={"origin":c["origin"],"size":c["size"],"uv":solid_uv(u,v)}
            if "pivot" in c:nc["pivot"]=c["pivot"]
            if "rotation" in c:nc["rotation"]=c["rotation"]
            nb["cubes"].append(nc)
        cgeo["bones"].append(nb); added.append(nb["name"])
    out_model=a.output_dir/f"{stem}.geo.json"; out_model.write_text(json.dumps(candidate,separators=(",",":"))+"\n",encoding="utf-8")
