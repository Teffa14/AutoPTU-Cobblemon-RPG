...
public final class IntegrationFeatureCompatibility {
    ...
    private static Map<Feature, Requirement> buildRequirements() {
        ...
        requirements.put(Feature.RUNTIME_RULE_STATE_SEED, requirement("Bind the reservation-scoped runtime preparation to canonical current injury counts only. AutoPTU-Java owns the battle round, injury-history rotation, Aura Break blocker selection, Aura Storm resolution and every subsequent hook mutation; none of those values may be supplied or derived by Minecraft.", UpstreamCompatibilityMatrix.Capability.FULL_STATEFUL_DAMAGE_PIPELINE, UpstreamCompatibilityMatrix.Capability.ABILITIES, UpstreamCompatibilityMatrix.Capability.FULL_TURN_ROUND_LIFECYCLE));
