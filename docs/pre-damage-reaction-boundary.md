...
- AutoPTU-Java: `7a7a6d93cedf82aa16e427b166160b6e39756676`
- AutoPTU Python: `4b35bc2b37b7f3e536c3974982729025740fcd79`
...
Current Java main includes the generic `PreDamageFollowUpMoveExecutor` seam, the parity-backed Sway consumer, and `ReactionPushApplication.pushToFirstOpenAdjacent`. Sway therefore has authoritative Java ownership for eligibility, recursion guard, once-per-use bookkeeping, STANDARD spend, redirect event generation, push destination legality and deterministic adjacent push selection.
...
Sway is not yet executable end-to-end through ordinary Java BattleRuntime resolution. `RuntimePreDamageReactionContextFactory` still constructs the live PRE-damage context without a follow-up executor. The registered Sway hook can therefore exist and unit-test its executor callback while the normal runtime path remains unable to perform the nested redirect. The adapter must keep Sway gated until Java main injects that executor through the authoritative runtime path.
...
Blocking for this slice: live BattleRuntime follow-up-executor injection for Sway; general push/pull/knockback/interception coverage; complete ability/reaction families; full stateful modifier coverage across abilities/items/terrain; AI tactical policy.
...
When AutoPTU-Java live-wires the follow-up executor into ordinary BattleRuntime PRE-damage context creation, prove adapter playback of the authoritative Sway redirect, nested semantic result, push relocation, recursion-guard cleanup, and original-hit cancellation without introducing Sway-specific legality in Minecraft.
