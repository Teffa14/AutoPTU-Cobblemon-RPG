...
/**
 * World-wide continuity for already-started Minecraft-native CALM routes on published wild actors.
 *
 * Ambient updates may revoke a native route while re-evaluating WATCHING/ALARMED/recovery priority.
 * This runtime may rehydrate only the same deterministic CALM segment destination when the actor is
 * still eligible, the habitat leash remains valid, no player guard is active and the remaining path
 * is still safe in current Minecraft geometry. It never supplies PTU movement or combat authority.
 */
public final class WildCalmNavigationContinuityRuntime implements ModInitializer {
    ...
    private void update(MinecraftServer server) {
        ...
        for (var projection : WildEcologyProjectionRegistry.collect(world)) {
            ...
            if (!navigation.isIdle()) {
                ...
                if (!remainingPathStillSafe(world, actor, currentPath, projection)) {
                    navigation.stop();
                    active.remove(actor.getUuid());
            ...
            if (dx * dx + dz * dz <= profile.calmStopDistance() * profile.calmStopDistance()) {
                active.remove(actor.getUuid());
                continue;
            }
    ...
    static boolean remainingPathStillSafe(
        ...
        for (int pathIndex = currentNodeIndex, profileIndex = 0;
            ...
            if (!WildCalmCollisionNavigationRuntime.stableCalmSurfaceNeighborhood(surfaceY, adjacentSurfaceY)) return false;
    ...
    private static boolean pathNodeVolumeClear(ServerWorld world, PokemonEntity actor, BlockPos node) {
        ...
        return WildCalmCollisionNavigationRuntime.presentationNodeClear(blockSpaceClear, activeWildOverlap);
    }
    ...
    static boolean shouldRehydrate(Long recordedSegment, long currentSegment, boolean calmActive, boolean nearbyPlayer) {
