...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "014933ea022198d5558a4f899ba4b41d0c59a47f";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.ACTION_ECONOMY_AND_INITIATIVE, verified(
                "ActionBudget, typed turn phases, deterministic initiative, Trick Room/League ordering; Trainer AP is separately authoritative in TrainerRuntimeState. InitiativeProgressState now stores server-owned initiative order/cursor and BattleRoundController owns lifecycle mutation. Analytic resolves against authoritative defender ActionBudget history plus InitiativeProgressState with Python-parity strict cursor semantics",
                "Treat action availability, initiative and Trainer resource spending as core-owned state. The integration may freeze canonical Trainer AP at battle start, but must not spend/restore AP or initialize/mutate ActionBudget on behalf of PTU lifecycle logic. Minecraft must never declare whether an Analytic target has already acted, replace initiative order, set the initiative cursor or supply initiative observations for the bonus."));
