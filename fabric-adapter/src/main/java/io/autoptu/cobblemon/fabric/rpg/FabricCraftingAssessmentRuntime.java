 ...
 *
 * <p>This command never rolls an outcome or consumes materials. It reports the server-owned task
 * definition against persistent canonical Trainer skills so a later workstation and atomic craft
 * transaction can reuse the same authority boundary without exposing a rerollable preview.</p>
 */
public final class FabricCraftingAssessmentRuntime {
    ...
    private static int assess(ServerCommandSource source, String requestedRecipe) {
        ...
        WorldTaskDefinition task = CATALOGUE.find(requestedRecipe).orElse(null);
        if (task == null) {
            String known = CATALOGUE.all().stream()
                    .map(WorldTaskDefinition::taskId)
