...
public final class CurrentUpstreamCompatibilityInspection {
    public static final String AUTOPTU_JAVA_SHA = "a2931ccc3dd37119a94445f44fb833c755d311c1";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.FULL_TURN_ROUND_LIFECYCLE,
                new Evidence(
                        ...
                        "The default production rollover call advances the round, derives and installs the next mixed Trainer/Pokemon initiative order only from BattleRuntimeState, applies initiative temporary-effect cleanup, resets the initiative cursor and opens the next actor turn without an adapter-supplied rebuilder. BattleRuntimeState owns BattleDelayedHitState, including the delayed queue and single Python-compatible battle RNG stream. DelayedHitLifecycleExecutor resolves due COMBATANT-target entries in insertion order through the ordinary authoritative move-resolution path. FieldRoundLifecycleHook is now registered in the built-in ROUND_START lifecycle before later round hooks, advances canonical terrain -> zones -> rooms through FieldRoundProgression, writes the resulting field state back to BattleEnvironmentState, removes Wonder Room cleanup statuses from authoritative combatants and emits FieldEffectEndedEvent semantic events.",
                        "Complete lifecycle remains broader than these bounded services. Delayed-hit execution is still not registered into the default ROUND_START flow on main, due TILE/area delayed hits remain unsupported, and Trainer AP/temporary-AP, send-out Features, Air Lock, Arena Trap, Intimidate, Impostor and other Python round-start behavior remain core-owned follow-up work. Minecraft must not advance field durations, remove Wonder Room statuses, own the delayed queue or mutable RNG, inject lifecycle hooks, or choose lifecycle ordering."));
        result.put(UpstreamCompatibilityMatrix.Capability.MOVE_SPECIFIC_BEHAVIOR,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "DelayedHitExecutionPolicy freezes target-resolution re-entry and the current target-binding contract: resolve_delayed_hits forwards both targetId and targetPosition unchanged; a canonical targetId remains COMBATANT targeting even when a stored targetPosition is present, with that position retained as the aim anchor, while a position-only delayed entry resolves as TILE. DelayedHitResourcePolicy freezes resource ownership, BattleRuntimeState owns BattleDelayedHitState and its RNG, and DelayedHitLifecycleExecutor resolves due COMBATANT-target entries in insertion order. RuntimeMoveResolution.applyDelayedUsingAuthoritativeCombatState re-derives effective metadata and combat inputs from current BattleRuntimeState before executing the normal PythonRandom, HP mutation, damage-history and MoveResolvedEvent path without a second action/frequency spend.",
                        "This is bounded delayed-hit execution, not complete delayed-move parity or the full move library. TILE/area delayed execution remains unsupported on main and automatic ROUND_START delayed-hit dispatch has not landed on main. Minecraft must not schedule or mature delayed hits, choose due entries, rewrite target mode because a target position exists, replace a canonical targetId with TILE targeting, supply RNG/combat inputs, consume or refund move frequency/actions, bypass target binding or invent tile/area semantics."));
        result.put(UpstreamCompatibilityMatrix.Capability.TERRAIN_WEATHER_HAZARDS_ZONES_REACTIONS,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "BattleEnvironmentState owns weather, PTU terrain identity, Tailwind teams, grounded state, mounted relationships, initiative ordering modes and duration-bearing FieldEffectEntry state for terrain, zones and rooms. FieldRoundLifecycleHook now executes FieldRoundProgression in the canonical ROUND_START registry, preserves terrain -> zones -> rooms ordering, mutates the authoritative environment, applies FieldStatusCleanupRequest for Wonder Room and emits FieldEffectEndedEvent semantic playback.",
