...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "6678d4563116a4ec8c70d9daafc00d28bb9ab25b";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.ACTION_ECONOMY_AND_INITIATIVE, verified(
                "ActionBudget, typed turn phases, deterministic initiative, Trick Room/League ordering; Trainer AP is separately authoritative in TrainerRuntimeState. InitiativeProgressState stores server-owned initiative order/cursor and BattleRoundController owns lifecycle mutation. BattleRoundController.advanceInitiativeTurn advances the canonical cursor, skips missing/fainted/inactive slots, resets only the selected combatant's consumed actions, opens START, emits TurnStartedEvent, resolves START status/ability/perk hooks, then consumes any pending status skip before returning the autobattler decision window. BattleRoundController.advanceInitiativeTurnWithRollover now detects exhausted initiative, runs authoritative ROUND_START first, invokes the core-owned InitiativeRoundRebuilder against the post-round-start BattleRuntimeState, installs only validated combatant IDs, and then continues actor selection. The complete initiative rebuild formula remains unported. Analytic resolves against authoritative defender ActionBudget history plus InitiativeProgressState with Python-parity strict cursor semantics",
                "Treat action availability, initiative, turn selection, START effect ordering, round rollover timing and Trainer resource spending as core-owned state. The integration may render Java-emitted lifecycle events in sequence, but must not choose the next actor, rebuild or replace initiative order, provide client-computed initiative, skip initiative slots, reset ActionBudget, set the initiative cursor, execute START hooks, consume a status skip, spend/restore AP, or declare whether an Analytic target has already acted."));
        entries.put(Capability.FULL_TURN_ROUND_LIFECYCLE, partial(
                "BattleRoundController, ordered LifecycleHookRegistry, StatusPhaseEffectRegistry, AbilityPhaseEffectRegistry, PerkPhaseEffectRegistry, CombatantPhaseEffectDispatcher STATUS -> ABILITY -> PERK ordering, structured StatusStateStore, active actor/phase state, authoritative phase/turn-start/turn-end events, advanceInitiativeTurn selection with Python parity, TURN_START START dispatcher execution before decision-window return, pending status-skip consumption after START effects, turn-end boundary, and advanceInitiativeTurnWithRollover orchestration that runs ROUND_START lifecycle events before core-owned InitiativeRoundRebuilder output and the next TurnStartedEvent. TemporaryEffectStore, move-frequency reset, selected round-start cleanup, canonical RoundInjuryHistoryState, server-owned BattleRuntimeState.currentRound and InitiativeProgressState, damage/injury history rotation, delayed-hit scheduling/binding, Flinch behavior, Strange Tempo, Lancer END, authoritative TrainerRuntimeState/controller binding, mutable CombatStageState and parity-backed Defense Mastery/fixed Link Feature END behavior are also present",
                "Consume verified lifecycle state/events only and preserve authoritative ordering, including round-start lifecycle events before the next turn_start and turn_start -> START status/ability/perk semantic effects -> pending status_skip before any adapter decision UI opens. Round advancement, initiative rebuild invocation/order, actor selection, initiative cursor advancement, inactive/fainted skipping, action reset, START hook execution and skip consumption remain Java-owned. The full Python initiative rebuild formula, trainer initiative entries, Tailwind/Bashed/temporary initiative modifiers, Trick Room/League rebuild policy, unported concrete START implementations, AP spending, broader perk/status/ability libraries, delayed-hit execution, terrain/zone/room advancement, send-out effects and remaining Python lifecycle hooks remain core-owned. The adapter must never set or advance BattleRuntimeState.currentRound or mutate InitiativeProgressState."));
        ...
        entries.put(Capability.TRAINER_FEATURES_AND_PERKS, partial(
                "Authoritative TrainerRuntimeState stores server-owned Trainer Feature identities and AP; BattleRuntimeState binds combatants to trainer controllers; PerkPhaseLifecycleHook derives owned Features from runtime state; PerkPhaseEffectRegistry now participates in TURN_START START dispatch as well as PHASE_CHANGE; CombatStageState and CombatStageHookRegistry are authoritative battle state/reaction seams; selected status-skip exceptions, Defense Mastery and fixed Attack/Defense/Special Attack/Special Defense/Speed Link END behavior are parity-backed; Link stage changes intentionally remain on their Python-parity direct mutation path",
                "Freeze and transport Trainer Feature ownership, battle-start AP and combatant-controller bindings only from canonical server state. Minecraft/client payloads may not grant Features, set AP, spend/restore AP, choose controllers, execute START perk hooks, mutate combat stages, run combat-stage reactions or execute perks. Remaining Trainer Feature/perk implementations stay AutoPTU-Java-owned."));
        entries.put(Capability.AI_LEGAL_ACTION_INFRASTRUCTURE, verified(
                "Runtime-authoritative autobattler action space, affiliation, geometry, action budget, move availability and frequency filtering; advanceInitiativeTurn resolves START effects and pending status skips before the AI receives its decision window. After initiative exhaustion, advanceInitiativeTurnWithRollover performs ROUND_START and accepts only a core-owned InitiativeRoundRebuilder result before opening the next decision window",
                "AI may choose only from legal choices produced by the core after authoritative rollover/START effects and skips have resolved. The adapter must not supply the next-round initiative order or expose a decision window from pre-rollover, pre-START or pre-skip state."));
