...
public final class CurrentUpstreamCompatibilityInspection {
    public static final String AUTOPTU_JAVA_SHA = "5d9e5069fa0c68432825a48be25fff6ba245d305";
    public static final String AUTOPTU_PYTHON_SHA = "743b0ff76c63d8ab2131fbf8de4e2e2430b9eea4";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.COMPLETE_STATUS_LIFECYCLE,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "StatusStateStore preserves ordered stacked StatusEntry values, including repeated normalized names when a rule explicitly stacks them. Name-based views remain unique. TrainerFeatureEffectRegistry has Python-parity apply_status/remove_status effects over that canonical store. AutoPTU-Java main now binds StatusAbilityPreventionResolver into live status application using target-owned RuntimeCombatantState ability suppression for Inner Focus/Flinch, Immunity/poison and Insomnia or Vital Spirit/sleep.",
                        "Selected prevention is live, but complete status ticking, expiry, cures, remaining immunities, source-sensitive behavior and interactions are still partial. Minecraft may persist and transport authoritative status entries but must not interpret or execute missing status rules."));
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.ABILITIES,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "Merged upstream PR #153 binds the declarative StatusAbilityPreventionResolver into live authoritative status application. RuntimeCombatantState owns abilitiesSuppressed; status requests cannot supply or override it. Inner Focus blocks Flinch, Immunity blocks poison, and Insomnia/Vital Spirit block sleep unless the target's abilities are suppressed.",
                        "Own Tempo, Oblivious, Run Away, attacker-side ignore_defensive_abilities, Safeguard, and the sources that create/remove ability suppression remain outside this verified family. The complete PTU ability library is not ported, and Minecraft must not decide status prevention or suppression, grant abilities or supply hook results."));

        result.put(UpstreamCompatibilityMatrix.Capability.TRAINER_FEATURES_AND_PERKS,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "TrainerRuntimeState owns Feature identities, base AP, temporary AP grants, ActionBudget, initiative inputs, explicit initiative Speed, team identity, generic Feature resources and usage/cooldown bookkeeping. TrainerFeaturePrerequisiteResolution, TrainerFeatureContextResolution, TrainerFeatureFrequencyResolution, TrainerFeatureResourceResolution and TrainerFeatureUsageResolution are Python-parity backed. TrainerFeatureExecutionService.executeAuthoritative commits generic bookkeeping only after the effect reports applied. TrainerFeatureTargetResolution provides Python-parity generic Pokemon target scopes and TrainerFeatureTrainerTargetResolution provides parity-backed trainer-target scopes. TrainerFeatureEffectRegistry includes heal/heal_active, raise_cs, grant_temp_hp, grant_ap, apply_status and remove_status. The inspected Python oracle at 743b0ff7 still performs guards and target/effect execution before resource consumption and usage/cooldown bookkeeping.",
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "Fabric 1.21.1 and Cobblemon 1.7.3 boot together in production-remapped dedicated-server CI. Live smokes verify UUID lookup, authoritative relocation, positive HP mirroring and public BATTLE_STARTED_PRE preemption. Player identity is authenticated through PlayerManager and durable player/profile/Pokemon state. ServerOwnedWildEncounterIdentityBinder now accepts only battle/side/actor/Pokemon identity keys from the Cobblemon signal and maps them to a preprovisioned server-owned CanonicalWildRoster with exact cardinality and alias checks. No live Pokemon values cross the binder. FileCanonicalPokemonRepository and FileCanonicalItemReservationRepository remain world-scoped durable authority stores; the two-process dedicated-server restart smoke verifies Trainer, encounter profile, Pokemon aggregate, item quantity and active item reservation across world reopen.",
                        "A successful authenticated graphical client encounter is still pending. A trusted server-owned encounter service must provision the WILD canonical participant, roster and battle values before this binder can resolve them. Encounter-profile creation, cross-aggregate transaction recovery, journaling, reservation expiry/reconciliation, RuntimeCombatantState materialization, zero-HP/faint presentation and complete battle playback remain pending. ServerPlayerEntity and Cobblemon state may provide identity/presentation inputs only and never PTU stats, inventory truth, modifiers, legality or outcomes."));
