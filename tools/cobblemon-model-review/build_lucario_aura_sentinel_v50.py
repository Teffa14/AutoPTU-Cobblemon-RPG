#!/usr/bin/env python3
"""Build Lucario Aura Sentinel V50 staging assets from the pinned official JAR.

Pre-production only. The builder validates the same-species dossier first, downloads
and verifies the current pinned compatible Cobblemon release, preserves the exact
87-bone official Lucario anatomy, and appends only authored ``ouros_*`` cosmetics.
It refuses to write into runtime production resources.
"""
...
POSER_SHA256 = "7cd9642b38fd1c3e2518cc7f30cd1ea221cac9c89e4b413551151418a4e3c07d"
RESOLVER_SHA256 = "a1785270f9f21378e6287b30e3e309de4daa348f21e33fcb8a8b03a134508e81"
MODEL_LICENSE_SHA256 = "fb8e971d1895863ec9fc5f3cfc526c64af980bd6c93d0a1615c7969df46a6660"
...
DOSSIER_PATH = Path("docs/cobblemon-skin-reference-dossiers/0448_lucario.json")
DOSSIER_VALIDATOR = Path("tools/cobblemon-model-review/validate_species_reference_dossier.py")
...
def load_release() -> tuple[dict, bytes]:
    version = json.loads(request_bytes(f"https://api.modrinth.com/v2/version/{VERSION_ID}").decode("utf-8"))
...
def unique_asset_by_hash(zf: zipfile.ZipFile, expected_sha256: str, suffixes: tuple[str, ...]) -> tuple[str, bytes]:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one asset for SHA-256 {expected_sha256}, found {len(matches)}")
    return matches[0]
...
def validate_patch(patch: dict) -> list[dict]:
    ...
    required = {
        ...
        "poserSha256": POSER_SHA256,
        "resolverSha256": RESOLVER_SHA256,
        "modelLicenseSha256": MODEL_LICENSE_SHA256,
...
def build(repo_root: Path, output_dir: Path) -> dict:
    validate_reference_gate(repo_root)
    ...
    report = {
        ...
        "promotionBlocker": "Deliberate independent derived normal and shiny paint, production resolver, professional manifest, registry promotion, full validators and generic Blockbench review are still required.",
        "source": {
            "modrinthProjectId": PROJECT_ID,
            "modrinthVersionId": VERSION_ID,
            ...
            "normalTextureSha256": NORMAL_SHA256,
            "shinyTexturePath": shiny_path,
            "shinyTextureSha256": SHINY_SHA256,
            "animationPath": animation_path,
            "animationSha256": ANIMATION_SHA256,
            "poserPath": poser_path,
            "poserSha256": POSER_SHA256,
            "resolverPath": resolver_path,
            "resolverSha256": RESOLVER_SHA256,
            "modelLicensePath": license_path,
            "modelLicenseSha256": MODEL_LICENSE_SHA256,
        ...
        "geometry": {
            ...
            "cosmeticCubeCount": sum(len(bone["cubes"]) for bone in appended),
    ...
    (output_dir / "build-report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    return report
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path(".professional-candidate/0448_lucario/v50"))
    ...
    print(json.dumps(build(root, output), indent=2, ensure_ascii=False))
