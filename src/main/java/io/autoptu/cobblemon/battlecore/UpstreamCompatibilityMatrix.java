...
public final class UpstreamCompatibilityMatrix {
    public static final String AUTOPTU_JAVA_SHA = "4e1492a642350e0d657dba8587e358a2f669b59c";
    ...
    private static Map<Capability, Entry> buildEntries() {
        ...
        entries.put(Capability.ACTION_ECONOMY_AND_INITIATIVE, verified(
                "ActionBudget, typed turn phases, deterministic initiative, Trick Room/League ordering, InitiativeProgressState, authoritative initiative assembly/installation, trainer initiative speed/entry resolution, and RuntimeInitiativePokemonCandidateFactory projection from BattleRuntimeState. BattleEnvironmentState supplies weather/terrain/Tailwind/grounded state; TrainerRuntimeState now owns Trainer Feature identities, AP and initiativeModifier; Agility Training, parental-bond-child and initiative-zero flags are read from canonical TemporaryEffectStore. BattleRoundController owns actor selection, START processing and rollover. Analytic resolves against authoritative defender ActionBudget history plus InitiativeProgressState with Python-parity strict cursor semantics",
        ...
        entries.put(Capability.FULL_TURN_ROUND_LIFECYCLE, partial(
                ...
                "Consume verified lifecycle state/events only and preserve authoritative ordering. Java now owns much more of initiative rebuilding inputs and installation, but complete Python start_round parity is still absent: broader terrain/zone/room progression, delayed-hit execution, send-out effects, remaining status/ability/perk hooks and other lifecycle behaviors remain incomplete. The adapter must never advance currentRound, mutate InitiativeProgressState, assemble initiative, run lifecycle hooks or manufacture missing round effects."));
        ...
        entries.put(Capability.TERRAIN_WEATHER_HAZARDS_ZONES_REACTIONS, partial(
                "Terrain movement costs, weather calculation primitives, BattleEnvironmentState for canonical weather/terrain/Tailwind/grounded inputs, generic HookSource categories and lifecycle seams exist",
