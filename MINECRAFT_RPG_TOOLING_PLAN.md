...
| WORLD-021 | BLOCKED | Gates/doors controlled by canonical badge/key/progression state — blocked until a durable server-owned badge/key/progression aggregate exists; Minecraft advancements, inventory items and client state must not be substituted as canonical progression truth. |
...
| WORLD-024 | LIVE/PARTIAL | Persistent world-event objects — PR #368 / merge `5d1910dfe25458da26fadfb2230e2b4ac5a5f165`; authenticated Trainers can activate an already-registered Ouros shrine through physical right-click or `/autoptu interact`, persisting world-scoped DORMANT -> ACTIVATED state exactly once through atomic revisioned storage. No PTU reward, progression, outcome or client truth is inferred; additional authored world-event object types remain follow-up work. |
...
| SVC-019 | BLOCKED/PARTIAL | Reconnect/restart active-session recovery. PR #310 / merge `cc7f4409662ca5580c038d3200ff4e398a11dbdc` provides durable pending post-battle transaction discovery/replay. Fabric startup/reconnect wiring is blocked until production supplies durable battle snapshot and committed-outcome repositories that survive restart; active unresolved battle sessions still require authoritative terminal semantics. |
