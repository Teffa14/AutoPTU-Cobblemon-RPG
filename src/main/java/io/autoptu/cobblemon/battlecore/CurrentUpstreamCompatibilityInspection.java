...
public final class CurrentUpstreamCompatibilityInspection {
    public static final String AUTOPTU_JAVA_SHA = "5f0df19c14e1e2a9b2bfc64522cf704b483e564e";
    public static final String AUTOPTU_PYTHON_SHA = "1cc0e5e449e0ad1745abf0d432892fd90998ef1c";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.FULL_STATEFUL_DAMAGE_PIPELINE,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "RuntimeMoveResolution owns canonical accuracy/damage, PRE/post hooks, sequential authoritative area resolution, HP/history mutation and declaration-level resource bookkeeping. RuntimeMoveResolutionWithFollowUps executes PRE-damage follow-up moves synchronously through the authoritative runtime. Java main 5f0df19 wires MoveSpecialPreDamageResolution into BattleRuntime before defender PRE-damage reactions, so core-owned PRE_DAMAGE result mutations are observed by the subsequent reaction and final HP/history mutation. StatusMoveRuntimeResolution provides the verified non-damaging Status execution branch without mutating HP or damage history.",
                        "Only PRE_DAMAGE move-special execution is live in the main damage pipeline. POST_DAMAGE and END_ACTION parity remain incomplete, and the open upstream POST_DAMAGE bridge is not promoted here. Minecraft must not dispatch hooks, mutate move-special result state, re-run damage, HP/history mutation, nested follow-up resolution or infer missing modifiers/effects."));
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.MINECRAFT_COBBLEMON_CRAFTICS_ADAPTER_PLAYBACK,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "Fabric/Cobblemon dedicated-server smoke coverage, authenticated identity, canonical reservations, live entity projection and generic semantic-event playback are present. The adapter may project authoritative zero-damage Status MoveResolvedEvent playback because Java owns declaration legality, accuracy result and resource spend. Live PRE_DAMAGE move-special execution remains wholly inside AutoPTU-Java BattleRuntime and exposes only authoritative result/state/events for downstream projection.",
