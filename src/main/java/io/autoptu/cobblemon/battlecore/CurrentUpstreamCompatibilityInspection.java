...
public final class CurrentUpstreamCompatibilityInspection {
    public static final String AUTOPTU_JAVA_SHA = "e5439ac27a77cc41300435ed352cf4baf41f1269";
    public static final String AUTOPTU_PYTHON_SHA = "ce82de564a09b4b66abebda356eca46a9723af4d";
    ...
    private static Map<UpstreamCompatibilityMatrix.Capability, Evidence> build() {
        ...
        result.put(UpstreamCompatibilityMatrix.Capability.COMPLETE_STATUS_LIFECYCLE,
                new Evidence(
                        UpstreamCompatibilityMatrix.Support.PARTIAL,
                        "StatusStateStore preserves ordered stacked StatusEntry values, including repeated normalized names when a rule explicitly stacks them. Name-based views remain unique. Upstream parity tests freeze append, first-match replacement, remove-one, remove-all and clear storage behavior against the Python list representation. TrainerFeatureEffectRegistry also has Python-parity apply_status/remove_status effects over that canonical store. StatusAbilityPreventionResolver now freezes a pure target-owned prevention contract for selected abilities without wiring it into live application.",
                        "This verifies representation/storage semantics plus bounded Trainer Feature status mutations and a declarative prevention resolver. Complete status ticking, expiry, cures, runtime prevention wiring, source-sensitive behavior and remaining interactions are still partial. Minecraft may persist and transport authoritative status entries but must not interpret or execute missing status rules."));
