...
import io.autoptu.cobblemon.authority.CanonicalStorageManagementService;
...
public final class FabricItemStorageTerminalRuntime {
    ...
    public static void register() {
        UseBlockCallback.EVENT.register((player, world, hand, hitResult) -> {
            ...
            serverPlayer.openHandledScreen(new SimpleNamedScreenHandlerFactory(
                    (syncId, inventory, ignored) -> new CanonicalItemStorageScreenHandler(syncId, inventory, serverPlayer, terminalPos),
    ...
    static boolean withinInteractionDistance(ServerPlayerEntity player, BlockPos pos) {
        return player.squaredDistanceTo(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D) <= MAX_INTERACTION_DISTANCE_SQUARED;
    }
    ...
    static final class CanonicalItemStorageScreenHandler extends GenericContainerScreenHandler {
        ...
        private final CanonicalStorageManagementService managementService = new CanonicalStorageManagementService();
        ...
        CanonicalItemStorageScreenHandler(int syncId, PlayerInventory inventory, ServerPlayerEntity player, BlockPos terminalPos) {
            this(syncId, inventory, player, terminalPos, new SimpleInventory(TOP_SLOT_COUNT));
        }

        private CanonicalItemStorageScreenHandler(int syncId, PlayerInventory inventory, ServerPlayerEntity player,
                                                  BlockPos terminalPos, SimpleInventory displayInventory) {
            ...
            this.bagQuery = new CanonicalBagQueryService(FabricCanonicalPlayerStoreRuntime.requireAssetRepository(player.getServer()));
        ...
        @Override public boolean canUse(PlayerEntity player) {
            return player == this.player && !player.isRemoved() && isItemStorageTerminal(player.getWorld(), terminalPos)
                    && player.squaredDistanceTo(terminalPos.getX() + 0.5D, terminalPos.getY() + 0.5D, terminalPos.getZ() + 0.5D)
                    <= MAX_INTERACTION_DISTANCE_SQUARED;
        }

        @Override public void onSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity clickingPlayer) {
            if (clickingPlayer != player || actionType != SlotActionType.PICKUP || button != 0) return;
            if (!canUse(clickingPlayer)) { player.closeHandledScreen(); return; }
            if (slotIndex < 0 || slotIndex >= TOP_SLOT_COUNT) return;
            BagSelection bag = bagSelections.get(slotIndex);
            if (bag != null) { depositOne(bag); return; }
        ...
        private boolean canonicalTrainerExists() {
        ...
        private void depositOne(BagSelection selection) {
            ...
            CanonicalStorageManagementService.Decision decision = managementService.canManage(
                    new CanonicalStorageManagementService.Request(playerId, canonicalTrainerExists(),
                            CanonicalStorageManagementService.Mutation.DEPOSIT, selection.itemInstanceId(), selection.templateId(), 1,
                            selection.revision(), selection.availableQuantity(), -1L, -1, bag, null));
            if (!decision.allowed()) {
            ...
            try {
                transferService.deposit("item-storage-ui:" + playerId + ":" + UUID.randomUUID(), playerId,
                        decision.itemInstanceId(), decision.quantity());
                player.sendMessage(Text.literal("Stored 1 " + decision.templateId() + "."), false);
        ...
        private void withdrawOne(StorageSelection selection) {
            ...
            CanonicalStorageManagementService.Decision decision = managementService.canManage(
                    new CanonicalStorageManagementService.Request(playerId, canonicalTrainerExists(),
                            CanonicalStorageManagementService.Mutation.WITHDRAW, null, selection.templateId(), 1,
                            -1L, -1, selection.storageRevision(), selection.quantity(), null, storage));
            if (!decision.allowed()) {
            ...
            try {
                transferService.withdraw("item-storage-ui:" + playerId + ":" + UUID.randomUUID(), playerId,
                        decision.templateId(), decision.quantity());
                player.sendMessage(Text.literal("Withdrew 1 " + decision.templateId() + "."), false);
            } catch (IllegalArgumentException | IllegalStateException failed) {
        ...
        private void refresh() {
            bagSelections.clear(); storageSelections.clear();
            ...
            for (CanonicalBagQueryService.BagEntry entry : bag.entries()) {
                ...
                displayInventory.setStack(bagSlot, menuItem(Items.LIME_DYE.getDefaultStack(),
                        "BAG " + entry.templateId() + " x" + entry.availableQuantity() + " · click to store 1"));
                bagSelections.put(bagSlot, new BagSelection(entry.itemInstanceId(), entry.templateId(), entry.availableQuantity(), entry.revision()));
                bagSlot++;
            }
            int storageSlot = STORAGE_START_SLOT;
            for (Map.Entry<String, Integer> entry : storage.quantities().entrySet().stream().sorted(Map.Entry.comparingByKey()).toList()) {
                if (storageSlot >= TOP_SLOT_COUNT) break;
                if (entry.getValue() <= 0) continue;
                displayInventory.setStack(storageSlot, menuItem(Items.CYAN_DYE.getDefaultStack(),
                        "STORAGE " + entry.getKey() + " x" + entry.getValue() + " · click to withdraw 1"));
                storageSelections.put(storageSlot, new StorageSelection(entry.getKey(), entry.getValue(), storage.revision()));
                storageSlot++;
            }
            displayInventory.markDirty(); sendContentUpdates();
        }

        private static ItemStack menuItem(ItemStack stack, String name) {
            stack.set(DataComponentTypes.CUSTOM_NAME, Text.literal(name)); return stack;
        }
    }
